class Admin::NewsfeedController < ApplicationController
  ...
  def index
    ...
    @news = NewsModel::all_for_clan(session[:user_clan])
  end
  ...
  def edit
    return unless require_acl('post_news') or require_clan_admin
    @newsitem = NewsModel.read_one(session[:user_clan], params[:id])
  end

  def destroy
    return unless require_acl('post_news') or require_clan_admin

    NewsModel::delete(session[:user_clan], params[:id])
  ...
  def create
    ...
    fields = {:title => params[:title],
              :short => params[:short],
              :long  => params[:long] }
    NewsModel::create(session[:user_clan], fields)
